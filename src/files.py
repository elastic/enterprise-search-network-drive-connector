...
from src.constant import DATETIME_FORMAT, ACCESS_ALLOWED_TYPE, ACCESS_DENIED_TYPE, ACCESS_MASK_DENIED_WRITE_PERMISSION, ACCESS_MASK_ALLOWED_WRITE_PERMISSION
...
class Files:
    ...
    def fetch_users_from_csv_file(self):
        ...
        """
        rows = {}
        if (os.path.exists(self.user_mapping) and os.path.getsize(self.user_mapping) > 0):
        ...
        if security_info.dacl:
            ...
            for ace in (aces or []):
                ...
                if not self.sid_to_username.get(sid):
                    self.logger.error(f"No mapping found for sid:{sid} in csv file. Please add the sid->user mapping for the {sid} and rerun the sync_user_permissions.py to sync the user mappings.")

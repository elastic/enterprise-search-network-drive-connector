...
from src.constant import DATETIME_FORMAT, IDS_PATH, DOCUMENT_SIZE
...
logger = log.setup_logging("network_drive_connector_index")


class Indexer(BaseClass):

    def __init__(self, start_time, end_time, checkpoint):
                ...
                for chunk in document_list:
                    ...
                    for each in response['results']:
                        if not each['errors']:
                            ...
                            logger.error("Unable to index the document with id: %s Error %s" % (each['id'], each['errors']))
                logger.info("Successfully indexed %s to the workplace out of %s" % (
                    total_documents_indexed, len(response['results'])))
        except Exception as exception:
            logger.exception("Error while indexing the files. Error: %s"
                             % (exception)
                             )
            self.is_error = True
    ...
    def indexing(self, drive, ids, storage, is_error_shared):
        ...
        if connection:
            files = Files(self.logger)
            document = files.fetch_files(connection, time_range)
            for doc in document:
                ids["files"].update({doc["id"]: doc['url']})
...
def init_multiprocessing(start_time, end_time, drive, ids, storage, is_error_shared, checkpoint):
    """This method initializes the IndexUpdate class and kicks-off the multiprocessing. This is a wrapper method added to fix the pickling issue while using multiprocessing in Windows
        ...
        """
    indexer = Indexer(start_time, end_time, checkpoint)
...
def start(indexing_type):
    ...
    while True:
        ...
        drive = data.get("network_drive.server_name")
        ...
        for num in range(0, worker_process):
            ...
            process = multiprocessing.Process(target=init_multiprocessing, args=(start_time_partition, end_time_partition, drive, ids_collection["global_keys"][drive], storage, is_error_shared, check))
            jobs.append(process)
